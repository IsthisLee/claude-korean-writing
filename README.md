...
<p align="center">
  ...
  평소 답변에 규칙을 붙이고, 글은 처음부터 자연스럽게 쓰고, 이미 쓴 글은 다듬고, .md 편집은 검사합니다.<br>
  원문은 이 컴퓨터 밖으로 나가지 않습니다.
</p>
...
> **v1.0.0**: 첫 릴리스입니다. 스킬 4종(처음 쓰기·윤문·글자 수·README 구성), `.md` 편집을 검사하는 훅, 문서 검사와 릴리스 스크립트를 담았습니다. 그 뒤 main에는 세션 시작에 상시 규칙을 넣는 훅이 더해졌고 아직 릴리스 전입니다. 상세: [CHANGELOG.md](CHANGELOG.md)

> **korean-writing은 Claude Code가 쓰는 모든 한국어의 품질을 맡습니다.** 설치하면 평소 답변부터 규칙이 붙습니다. **"운영팀에 보낼 안내문 써줘"**처럼 글을 써 달라고 하면 스킬이 스스로 로드되어 처음부터 자연스러운 한국어로 씁니다. `.md` 파일을 고치면 훅이 번역투와 AI 관용구를 잡아 알려줍니다.
...
왼쪽이 틀린 것은 아닙니다. 다만 사람이 한국어로 쓸 때는 나오지 않는 표현입니다. 이 플러그인은 그런 문장이 나오는 자리마다 하나씩 붙습니다.

| 자리                   | 붙는 것                           | 하는 일                                                                     |
| ---------------------- | --------------------------------- | --------------------------------------------------------------------------- |
| 평소 답변을 쓸 때      | SessionStart 훅(상시 규칙 주입)   | 세션이 열릴 때 규칙 요약을 한 번 넣습니다. 스킬이 뜨지 않는 답변을 맡습니다 |
| 글을 처음 쓸 때        | `korean-writing` 스킬             | 글 작성 요청이면 어디에 쓸 글이든 로드되어 처음부터 규칙을 적용합니다       |
...
| 검증 데이터 | 15문장 | 실제 위반 10건, 정상 5건. 회귀 테스트 63건                                                  |

## 왜 이 플러그인인가

**처음부터 잡습니다.** 다 쓰고 고치려 하면 구조가 이미 번역투로 잡혀 있어 안 고쳐집니다. 그래서 세션이 열릴 때 규칙을 넣고 글 작성 요청에는 스킬을 붙입니다. 사후 윤문은 보조 수단입니다.

**규칙은 실제 실패 문장으로만 만듭니다.** 정답 데이터 10건은 Claude Code가 실제로 생성한 문장이고 합성 예문이 없습니다. 반대로 실제 문서 143개에 돌려 사람이 쓴 글만 잡던 규칙 둘은 뺐습니다. 근거 없이 임계를 만지지 않습니다.

**오탐을 미탐보다 무겁게 봅니다.** 검사가 정상 작업을 막으면 사람이 검사를 꺼 버립니다. 실제 문서 143개에서 사람이 쓴 글이 걸린 것은 1개(0.7%)입니다.

**원문이 나가지 않습니다.** 훅은 bash와 python3 정규식이고 글자 수 스크립트는 `node:fs`만 씁니다. 네트워크 호출이 한 건도 없고, [SECURITY.md](./SECURITY.md)에 그것을 직접 확인하는 명령 세 개를 적어 두었습니다.

**끄기 쉽습니다.** 파일 하나, 세션, 상시 규칙, 플러그인 전체 네 단위로 끕니다. 훅은 편집을 되돌리지 않고 알리기만 합니다.

**모델이 바뀌어도 남는 안전망입니다.** 지금 모델은 스킬 없이도 꽤 깨끗하게 쓰지만, 몇 달 전 산출물에는 줄표가 문서 하나에 수백 개씩 있었습니다. 훅은 산출물을 보므로 어느 모델이든 같은 기준으로 잡습니다.
...
설치하면 끝입니다. 설정할 것이 없습니다. 새 버전은 `claude plugin update korean-writing`으로 받습니다.
...
| `python3`   | 훅의 판정, 테스트      | 훅이 검사 없이 통과합니다 |
| `node` 18+  | 글자 수 스크립트       | 그 스킬만 못 씁니다       |

별도 패키지는 설치하지 않습니다. macOS와 Linux는 CI가 같은 검사를 돌립니다. Windows는 Git Bash나 WSL이 필요하고 확인하지 않았습니다.
...
| ---------------------------- | --------------------------------------------------------- | -------------------------------------------- |
| 글을 처음부터 잘 쓰기        | "운영팀에 보낼 옵션 변경 안내문 써줘. 슬랙에 캐주얼하게." | `/korean-writing`                            |
...
평소 답변은 따로 부를 것이 없습니다. 세션이 열릴 때 규칙이 들어가 있습니다.
...
이미 써 둔 문서를 통째로 검사하려면 `scripts/check.sh 파일...`을 씁니다. 훅과 같은 기준으로 보고 걸린 파일이 있으면 종료 코드 1을 냅니다. CI와 pre-commit에서 그대로 쓸 수 있습니다. 설치 경로는 `claude plugin list`의 Path에 나옵니다.
...
| 범위          | 방법                                                                           |
| ------------- | ------------------------------------------------------------------------------ |
| 파일 하나     | 파일 머리에 `<!-- korean-writing: ignore -->`. 계약서처럼 격식이 요건인 문서용 |
| 세션 전체     | 환경변수 `KOREAN_WRITING_HOOK_DISABLED=1`                                      |
| 상시 규칙만   | 환경변수 `KOREAN_WRITING_ALWAYS_ON_DISABLED=1`                                 |
...
```
hooks-handlers/always-on.md     1.4 KB   세션마다 한 번. 답변용 규칙 요약
SKILL.md                        8.0 KB   글 작성 요청에 로드. 원칙·품질 기준·교정 예시
...
파일 전체가 아니라 이번에 쓴 부분만 보는 이유는 소음입니다. 전체를 보면 예전 표현이 매 편집마다 다시 걸립니다. 표를 통째로 빼는 이유는 이 README 때문입니다. 위 전/후 교정표의 '전' 칸에 있는 나쁜 예가 위반으로 잡혔습니다. 편집분이 영어 위주여도 한국어 문단이 있으면 그 줄들만 모아 같은 기준으로 봅니다. 영어 줄은 빠지므로 영어 산문의 줄표는 세지 않습니다.

줄표만은 파일 전체를 봅니다. 문단 하나씩 고치는 동안 파일에 줄표가 34개, 66개, 207개 쌓인 실제 문서가 있었는데 편집분만 세면 한 번도 안 걸리기 때문입니다. 이번 편집에 줄표가 없으면 파일에 아무리 많아도 잡지 않습니다.
...
| 글 작성 요청(슬랙·보고서·메모·README) | `korean-writing` 스킬  | 걸립니다                                               |
...
**외부 출판사 편집부의 지적이 근거입니다.** 단행본 초고 검토에서 줄표 삽입구와 `이깁니다` 류를 "요즘 원고에 공통적으로 나온다. 잘못된 표현은 아니지만 AI 생성 의심을 살 수 있다"고 짚었습니다. 서로 다른 저자의 원고 2건에서 같은 자리에 같은 `이깁니다`가 나오기도 했습니다. 문법적으로 맞아도 AI 서명처럼 굳은 표현은 피합니다. 그것이 이 플러그인의 기준입니다.

**넣고 빼는 것은 실측으로 정합니다.** `K6`은 규칙집보다 엄격해서 임계를 올렸고, `K7`의 "죽다"는 서버가 죽다·프로세스가 죽다처럼 개발자가 늘 쓰는 말이라 뺐고, `K8`의 `~에 대해`·`~를 통해` 횟수는 실제 문서에서 사람이 쓴 글만 잡아서 뺐습니다. 줄표는 편집분만 세면 쌓이는 것을 못 잡아 파일 전체로 넓혔습니다. 결정마다 숫자가 [`EVALUATION.md`](./EVALUATION.md)에 있습니다.
...
실문서 오탐은 이 플러그인과 무관하게 한 컴퓨터에 쌓인 한국어 `.md` 143개를 통째로 넣어 쟀습니다. 60개가 걸렸는데 59개는 2026년에 Claude가 쓴 문서(CLAUDE.md, 구현 로그, QA 보고서. 대부분 줄표)였고 사람이 쓴 문서는 1개였습니다. 같은 측정을 `scripts/measure.sh`로 분기마다 다시 합니다.

상시 규칙의 효과는 프롬프트 12개에 조건 2개와 표본 2개로 48건을 생성하고 같은 모델이 순서를 바꿔 두 번씩 블라인드로 판정해 쟀습니다. 24쌍에서 주입 쪽이 21승 3무 0패였고 12개 프롬프트 전부에서 이기거나 비겼습니다. 문체를 빼고 기술 오류만 따로 본 검사에서는 심각한 오류가 양쪽 0건입니다. 자세한 것은 EVALUATION.md의 C5에 있습니다.

스킬 유무를 같은 프롬프트 4개로 비교한 결과는 EVALUATION.md의 C4에 있습니다. 2026-09-10의 claude-opus-5에서는 두 조건 모두 훅을 통과해 생성 단계 효과를 이 표본으로 구분하지 못했습니다.
...
├── .gitattributes                    셸 스크립트를 LF 로 고정. CRLF 면 훅이 죽습니다
...
│   └── test_sessionstart.py          회귀 20건. 주입 내용·끄기·안전 종료
...
| 도구                                                        | 그쪽은                                                                                                                                | 이 플러그인은                                                                                                                                                               |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [im-not-ai](https://github.com/epoko77-ai/im-not-ai)        | 이미 쓴 한글에서 AI 티를 걷어내는 윤문 스킬입니다. 진단·윤문·마무리를 나눈 다중 호출 파이프라인이고 Claude Code 밖의 CLI도 지원합니다 | 그쪽의 AI 티 분류 체계를 규칙집으로 씁니다. 이 플러그인은 처음 쓸 때 규칙과 편집 훅, 상시 규칙에 무게를 두고 윤문은 단일 패스입니다                                         |
...
**A.** 아닙니다. 정답 10건으로 패턴을 만들었으니 그 10건이 잡히는 것은 당연하고 이 숫자는 고치다 깨뜨리지 않았는지 보는 회귀용입니다. 의미 있는 숫자는 오탐 쪽입니다. 한 컴퓨터에 쌓인 한국어 문서 143개를 통째로 넣어 60개가 걸렸습니다. 59개는 2026년에 Claude가 쓴 문서였고 사람이 쓴 문서는 1개(0.7%)였습니다. 정규식은 알려진 패턴만 잡으므로 새로운 어색함은 놓칩니다.
...
**A.** 아닙니다. 훅은 `python3` 정규식이고 글자 수 스크립트는 `node:fs`만 씁니다. 네트워크 호출이 한 건도 없습니다. [SECURITY.md](./SECURITY.md)에 그것을 직접 확인하는 `grep` 명령 세 개가 있습니다. 원문을 외부 서버로 보내는 `korean-spell-check`를 가져오지 않은 이유이기도 합니다.
...
**A.** 격식 문서(계약·약관·법률)면 파일 머리에 `<!-- korean-writing: ignore -->`를 넣습니다. 그 파일은 다시 알리지 않습니다. 세션 전체를 끄려면 `KOREAN_WRITING_HOOK_DISABLED=1`입니다. 패턴 자체가 틀렸으면 「어색한 문장 제보」 양식으로 보내 주세요. 그 문장이 `hooks-handlers/test_posttooluse.py`의 오탐 케이스가 됩니다. 훅째 끄려면 `claude plugin disable korean-writing`입니다.
...
**A.** 상시로 드는 것은 스킬 설명 넷(약 290토큰)과 답변용 규칙(약 660토큰)이고 합쳐서 약 950토큰입니다. 규칙은 세션마다 한 번 들어가 프롬프트 캐시에 올라가므로 턴마다 다시 내지 않습니다. 실측한 턴당 비용 차이는 0.0002달러입니다. 스킬 본문은 글 작성 요청이 있을 때만 로드되고 훅은 LLM을 부르지 않는 정규식입니다. 답변마다 다시 검토하는 Stop 훅은 그래서 두지 않았습니다.
...
<details>
<summary><b>Q7. 설치했는데 스킬이 안 보여요.</b></summary>

**A.** `claude plugin list`에서 `korean-writing`이 `enabled`인지 봅니다. 저장소를 `~/.claude/skills/`에 링크해 두고 마켓플레이스로도 설치했다면 설치본이 우선하고 링크 사본은 로드되지 않습니다. 둘 중 하나만 두세요. 새 세션에서 스킬 목록이 갱신됩니다.

</details>

<details>
<summary><b>Q8. Windows에서 되나요?</b></summary>

**A.** 확인하지 않았습니다. 훅이 bash 스크립트라 Git Bash나 WSL이 필요합니다. `.gitattributes`가 스크립트를 LF로 고정해 두어 CRLF 체크아웃으로 훅이 죽는 문제는 막아 두었습니다. 돌려 본 결과를 이슈로 알려 주시면 표에 넣겠습니다.
...
저장소를 `~/.claude/skills/`로 링크하면 `korean-writing@skills-dir`로 자동 로드됩니다. 마켓플레이스를 거치지 않으므로 고치는 즉시 반영됩니다. 마켓플레이스로 설치한 사본이 있으면 그쪽이 우선하니 개발 전에 `claude plugin uninstall korean-writing`으로 빼 주세요.
...
규칙집은 [im-not-ai](https://github.com/epoko77-ai/im-not-ai)에서 시작해 [claude-forge](https://github.com/sangrokjung/claude-forge)를 거쳐 왔습니다. Forge 판 470줄 중 406줄이 im-not-ai 원문과 같고, D-8·D-9와 J-3의 S1 상향은 Forge가 더한 것입니다. 윤문 스킬과 압축 룰북은 claude-forge, 글자 수 스크립트는 [k-skill](https://github.com/NomaDamas/k-skill), README 스킬은 [agent-toolkit](https://github.com/softaworks/agent-toolkit)에서 가져왔고 그 원 저작은 [agent-skills](https://github.com/joshuadavidthomas/agent-skills)입니다. `korean-writing` 스킬, 상시 규칙, 훅의 K2~K8, 실제 실패 문장으로 만든 검증은 여기서 썼습니다.
