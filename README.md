...
한국어 산문 품질 도구는 세 층으로 놓입니다. 규칙집을 만드는 쪽, 그 규칙을 특정 편집기에 붙이는 쪽, 편집기 전체를 꾸리는 프레임워크입니다. 이 플러그인은 가운데에 있습니다. 규칙집과 압축 룰북은 im-not-ai에서 그대로 가져왔고, Claude Code에 붙이는 방식은 여기서 만들었으며, 프레임워크가 필요하면 claude-forge가 있습니다.
...
| [claude-forge](https://github.com/sangrokjung/claude-forge) | 에이전트·커맨드·훅·규칙을 묶은 Claude Code 프레임워크. 한국어 산문 가드레일은 그중 한 부분입니다                    | 가져온 것은 없습니다. Forge에도 im-not-ai 계열 규칙집과 줄표 훅이 있어서, 같이 쓰면 줄표 검사가 겹칩니다                                           |
