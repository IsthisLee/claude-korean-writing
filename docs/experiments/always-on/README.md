...
30턴을 쌓은 뒤에도 주입이 듣는지 잰 장비입니다.

| 파일               | 무엇                                                              |
| ------------------ | ----------------------------------------------------------------- |
| `run.sh`           | 세션 하나를 열어 채움 30턴으로 컨텍스트를 키움                    |
| `fresh.sh`         | 그 세션을 이어 받아 한 번도 안 나온 프롬프트를 물음               |
| `judge.sh`         | 같은 깊이의 두 조건을 순서를 바꿔 두 번 판정                      |
| `settings-on.json` | 주입 조건. matcher 가 `startup` 뿐이라 이어 받을 때 재주입하지 않음 |

```bash
./run.sh on 30 && ./run.sh off 30   # 두 조건을 각각 채운다
./fresh.sh on && ./fresh.sh off     # 새 프롬프트를 던진다
./judge.sh                          # 판정
```
