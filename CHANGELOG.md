...
- **README 의 「부탁하는 법」이 저절로 뜨는 조건과 직접 부르는 명령을 한 표에 놓습니다.** 훅 둘은 부를 이름이 없고 검사는 요청 없이도 돈다는 것이 표에서 바로 보입니다. 「검증」에는 「자리별로 얼마가 드나」를 더해 상시 토큰·요청 때 드는 것·시간·품질 근거를 나란히 놓았습니다. 품질 숫자는 자리마다 잣대가 달라 줄 세울 수 없다는 것을 먼저 적었습니다. 영어판도 같이 고쳤습니다
