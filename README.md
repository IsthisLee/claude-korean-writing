...
> **v1.0.0**: 첫 릴리스입니다. 스킬 넷, `.md` 편집을 검사하는 훅, 문서 검사와 릴리스 스크립트가 들어 있습니다. 릴리스 뒤 main에는 세션이 열릴 때 답변용 규칙을 넣는 훅이 더해졌습니다. 상세: [CHANGELOG.md](CHANGELOG.md)
...
Claude Code의 한국어는 문법이 틀리지 않습니다. 그런데도 읽으면 걸립니다. 영어 문장을 그대로 옮긴 어순, 영어에서 건너온 비유, 어느 문서에서나 같은 자리에 나오는 상투구 때문입니다. 출판사 편집부가 그런 표현을 보고 원고를 AI가 썼다고 의심할 정도입니다.
...
문서와 설정은 서로 겨루지 않습니다. 우선한다, 따른다라고 씁니다. 출판사 편집부가 서로 다른 저자의 원고 두 건에서 같은 자리에 같은 "이깁니다"를 보고 지목한 표현입니다.

> 원인은 힙 부족이 아니었습니다 **—** 실측해보니 **—** 설정이 아예 먹히지 않았습니다.

줄표로 끼워 넣는 삽입구는 영어 문장 부호를 그대로 옮긴 것입니다. 같은 편집부가 지목한 나머지 하나이고, 한국어 산문에서 가장 눈에 띄는 표지입니다.
...
한 번의 패스로 끝냅니다. 읽고, 압축 룰북 `references/quick-rules.md`로 훑고, 판정이 애매한 곳만 규칙집을 열어 보고, 걸린 구간만 고치고, 자기 점검을 돌린 뒤 돌려줍니다. 한글 5,000자를 넘는 글은 논리 단위로 잘라 구간마다 돌립니다. 8,000자를 넘거나 정확도가 특히 중요하면 한 번의 패스로는 모자란다고 알립니다.

어기면 되돌리는 철칙이 넷 있습니다. 사실과 주장, 숫자, 날짜, 고유명사, 인용은 하나도 바꾸지 않습니다. 룰북이나 규칙집의 패턴에 걸린 구간 밖은 건드리지 않습니다. 장르와 문체는 원문 그대로 둡니다. 변경률이 30%를 넘으면 경고 신호로 보고, 50%를 넘으면 결과를 내놓지 않습니다.
...
**숫자 없이는 규칙을 바꾸지 않습니다.** 패턴 하나를 넣거나 빼거나 임계를 옮길 때마다 실제 문서에 돌린 결과가 있어야 합니다. 그렇게 바꾼 열세 건의 기록이 [`EVALUATION.md`](./EVALUATION.md)에 있습니다. 승패 의인화는 규칙집이 1회까지 허용하는데 훅이 1회에 걸려 임계를 2로 올렸습니다. 사물 의인화에서 "죽다"를 뺀 것은 서버가 죽었다, 프로세스가 죽었다가 개발자의 일상어이기 때문입니다. 번역투에서 `~에 대해`와 `~를 통해`의 횟수를 빼니 실제 문서에서 사람이 쓴 글만 잡던 규칙이 사라졌습니다.
...
- 상시 규칙의 효과 측정은 단일 턴이었습니다. 긴 대화에서 효과가 약해지는지는 재는 중입니다.
- **서브에이전트는 이 플러그인이 맡지 못합니다.** 세션 시작 훅의 주입, 서브에이전트 시작 훅의 출력, 스킬 목록 셋 다 서브에이전트에 전달되지 않는 것을 확인했습니다. 전달되는 것은 `CLAUDE.md` 하나입니다. 서브에이전트까지 맡으려면 아래 한 줄을 쓰는 쪽 `CLAUDE.md` 에 넣으세요.
...
한국어 산문 품질 도구는 세 층으로 놓입니다. 규칙집을 만드는 쪽, 그 규칙을 특정 편집기에 붙이는 쪽, 편집기 전체를 꾸리는 프레임워크입니다. 이 플러그인은 가운데에 있습니다. 규칙집은 im-not-ai에서 왔고, Claude Code에 붙이는 방식은 여기서 만들었으며, 프레임워크가 필요하면 claude-forge가 있습니다.
...
나머지는 이 저장소에서 썼습니다. `korean-writing` 스킬, 상시 규칙, 검사 훅의 `K2`부터 `K8`, 정답 데이터, 검증 기준이 그것입니다. 가져온 파일의 라이선스는 전부 MIT이고 원 저작권 표시는 [`NOTICE.md`](./NOTICE.md)에 모아 두었습니다. 이 저장소의 라이선스도 [MIT](./LICENSE)입니다.
