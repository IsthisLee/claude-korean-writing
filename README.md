...
<p align="center">
  <strong>Claude Code가 쓰는 모든 한국어가 사람이 쓴 것처럼 나옵니다.</strong><br>
  평소 답변, 새로 쓰는 글, 이미 쓴 글, 저장하는 <code>.md</code>. 한국어가 나오는 네 자리를 플러그인 하나가 맡습니다.<br>
  설치는 두 줄, 설정할 것은 없습니다. 원문은 이 컴퓨터를 떠나지 않습니다.
</p>
...
| 원인은 힙 부족이 아니었습니다 **—** 실측해보니 **—** 설정이 아예 먹히지 않았습니다.                             | 원인은 힙 부족이 아니었습니다. 실측해 보니 설정이 아예 먹히지 않았습니다.                              | 영어 문장 부호를 옮긴 줄표 삽입구. 실제 문서 143개에서 걸린 60개 대부분이 이것 |
...
상시 규칙이 평소 답변을 어떻게 바꾸는지는 같은 질문을 주입 없이 한 번, 주입한 채로 한 번 물어 보면 됩니다. 아래는 디바운스 훅을 설명해 달라는 같은 질문에 claude-sonnet-5가 낸 두 답의 같은 대목입니다. 원문 전체와 질문은 [docs/samples/](./docs/samples/)에 있습니다. 표본 하나의 예시이고 효과의 측정은 「검증」 절의 블라인드 판정입니다.
...
| 써 둔 글에서 번역투 걷어내기   | `/korean-writing:humanize [글 또는 파일 경로]` |
| 방금 윤문한 결과를 다시 다듬기 | `/korean-writing:humanize-redo [지시]`         |
...
파일은 1.5KB, 한글 402자이고 토큰으로 726입니다. 프롬프트 캐시에 들어가므로 턴마다 다시 내는 비용은 없습니다. 막는 일은 하지 않습니다. 규칙 파일이 없거나 읽을 수 없어도 종료 코드는 0입니다. 회귀 테스트 22건은 주입된 내용, 열 항목이 다 남아 있는지, 한글 420자 상한, 끄기 두 종, 안전 종료를 봅니다.
...
원칙은 여섯 가지입니다. 처음부터 깨끗하게 씁니다. 걷어내는 대상은 기계성뿐이라 격식, 전문성, 장르, 논지, 사실은 건드리지 않습니다. 원문에 없던 비유나 수사를 보태지 않습니다. 당위를 단정으로, 추측을 단정으로 바꾸지 않습니다. 다 쓰고도 미달이면 `humanize-korean`으로 보정합니다. 어떤 모델로 쓸지는 이 규칙이 정하지 않습니다.
...
나머지는 문장, 결말, 서식으로 나뉩니다. 문장에서는 주어를 사람이나 조직으로 두고, 만능 동사로 받지 않고, 한국어에 실제로 있는 비유만 쓰고, 구조를 은유로 설명하지 않고, 승패로 비유하지 않고, 대명사는 받을 것이 있을 때만 쓰고, 조사를 겹치지 않고, 수식을 명사 앞에 쌓지 않고, `것` 구문을 줄이고, 한 문체로 끝까지 가고, 같은 종결어미를 네 문장 넘게 잇지 않고, 문장 길이를 섞습니다. 결말에서는 결산 어휘와 `~하는 이유다`, `향후`, `과제도 남아 있다` 같은 공식을 쓰지 않고 할 말이 끝난 자리에서 끝냅니다. 서식에서는 줄표 삽입구를 두 번까지, 불릿은 정말 목록일 때만, 기계적 병렬과 콜론 부제 헤딩과 이모지를 피합니다.

보내기 전에 보는 것은 일곱 가지입니다. 부정 대구가 셋 이상인지, 연결어미 뒤에 쉼표를 찍었는지, 줄표가 셋 이상인지, 마지막 문단이 결산 공식으로 닫히는지, `축`·`갈래`·`결`·`레이어`를 합쳐 세 번 이상 썼는지, 사물이 사람처럼 움직이는 문장이 있는지, 소리 내어 읽었을 때 걸리는 데가 있는지. 마지막이 가장 무겁습니다. 계약서와 약관, 법률 문서, 공문은 딱딱함이 요건이라 이 규칙 밖에 있고 코드와 로그, 명령어, 직접 인용, 고유명사, 영어 원문에도 손대지 않습니다.
...
skills/humanize-korean/SKILL.md 29.2 KB   윤문 요청에
...
| 상시 컨텍스트 비용        | 약 1,450토큰 (상시 규칙 726 + 스킬 설명 넷 430 + 에이전트 셋 297) |
| 외부 네트워크 호출        | 0                                                                 |
| 회귀 테스트               | 63 / 63                                                           |
...
GitHub Actions는 push와 PR마다 macOS와 Linux에서 같은 검사를 돌립니다. 매니페스트와 이슈 양식의 문법, 훅의 실행 비트, 회귀 테스트 둘, 한국어 문서 12종이 자기 훅을 통과하는지, 글자 수 스크립트의 스모크 테스트, 내장한 윤문 스크립트의 컴파일과 실행 확인입니다. 여기에 shellcheck, `plugin.json`과 README 배지와 CHANGELOG의 버전이 같은지, `claude plugin validate`가 더해집니다.
...
상시로 드는 것은 스킬 설명 넷 약 430토큰(내장한 윤문 스킬 230 포함), 에이전트 셋 약 300토큰, 세션당 한 번 들어가는 상시 규칙 726토큰입니다. 상시 규칙은 설치된 플러그인을 그대로 두고 상시 규칙만 껐다 켜며 `claude -p`의 입력 토큰 총량을 뺀 값으로, 끄면 25,944, 켜면 26,670이었습니다. MCP 도구 정의와 사용자 설정을 빼야 값이 고정됩니다. 규칙은 프롬프트 캐시에 들어가 턴마다 다시 내지 않습니다. 스킬 본문은 글 작성 요청이 있을 때만 로드되고 훅은 LLM을 부르지 않습니다.
...
그런 뜻이 아닙니다. 정규식은 그 열 문장을 보고 만들었으니 열 문장을 잡는 것은 당연하고 그 숫자는 규칙을 고치다 무엇을 깨뜨리지 않았는지 보는 회귀용입니다. 봐야 할 숫자는 오탐입니다. 실제 문서 143개 중 사람이 쓴 글에서 걸린 것은 하나입니다. 정규식은 알려진 열 가지 패턴만 잡고 새로운 어색함은 잡지 못합니다.
