...
검사 훅은 `Edit`, `Write`, `MultiEdit`가 `.md` 파일을 고친 직후에 돕니다. 이번에 쓴 부분만 봅니다. 전체를 보면 예전 표현이 편집마다 다시 걸려 소음이 되기 때문입니다. 판정 방법은 다음 절에 있습니다.
...
이 숫자가 처음부터 나온 것은 아닙니다. 규칙을 늘려 AI 티 표지를 표본당 1.95에서 0.25로 줄인 판이 첫 판정에서는 옛 판을 이기지 못했습니다. 판정자가 지적한 것은 표지가 아닌 리듬과 소제목과 말미의 안내 문구였고, 그 셋을 규칙으로 넣고 나서 49승 0패가 됐습니다. 정규식으로 센 표지 개수는 품질의 근거가 되지 못합니다.
