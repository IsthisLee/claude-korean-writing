...
| 구성          | 수     | 무엇                                                                                            |
| ------------- | ------ | ----------------------------------------------------------------------------------------------- |
| 스킬          | 3      | `korean-writing`, `korean-character-count`, `crafting-effective-readmes`                        |
| 훅            | 2      | 세션 시작에 규칙 주입, `.md` 편집 직후 검사                                                     |
| 검사 패턴     | 8      | `K1`~`K8`. 줄표, 추상 구조어, 것 구문, AI 관용구, 기계적 병렬, 승패 의인화, 사물 의인화, 번역투 |
| 의존 플러그인 | 1      | im-not-ai의 `humanize-korean`. 이미 쓴 글의 윤문을 맡고, 설치할 때 같이 설치됩니다              |
...
`claude plugin list`에 `korean-writing`이 `enabled`로 보이면 됩니다. 윤문을 맡는 im-not-ai의 `humanize-korean`이 의존성으로 같이 설치되어 목록에 함께 보입니다. 1.1.0 이하를 설치해 둔 경우 `claude plugin install korean-writing`을 다시 실행하면 의존성이 채워집니다. 새 버전은 `claude plugin update korean-writing`으로 받습니다. 받아 둔 저장소 경로를 마켓플레이스로 등록해도 됩니다. 사내 사본이나 포크를 쓸 때입니다.
...
| 윤문 플러그인만 | `claude plugin disable humanize-korean`. korean-writing이 켜져 있는 동안은 막히고, 둘 다 끄는 명령을 오류 메시지가 알려 줍니다 |
...
| 써 둔 글 다듬기 | im-not-ai의 `humanize-korean` 스킬 (의존 플러그인) | 다듬어 달라는 요청이 오면       | im-not-ai 저장소의 `skills/humanize-korean/` |
...
이전 판에 있던 이 저장소의 윤문 스킬과 규칙집 사본은 뺐습니다. 같은 이름의 스킬이 둘 뜨는 충돌과, 원본이 바뀔 때마다 사본을 갱신하는 일을 없애기 위해서입니다. 대신 의존 플러그인이 세션마다 스킬 설명과 에이전트 9종의 설명으로 약 1,130토큰을 더 씁니다. 이 플러그인이 im-not-ai를 붙이는 목적은 어색한 번역투를 자연스러운 한국어로 바꾸는 것입니다. im-not-ai의 트리거 목록에는 탐지기 우회 표현도 있지만, 이 저장소는 그렇게 설명하지 않습니다.
...
**늘 읽히는 것은 작게 둡니다.** 평소에 컨텍스트에 들어가는 것은 상시 규칙과 스킬 설명 셋, 그리고 의존 플러그인의 스킬·에이전트 설명뿐이고, 큰 파일은 그 일이 생겼을 때만 열립니다.

```
hooks-handlers/always-on.md     1.4 KB   세션마다 한 번
SKILL.md                        8.3 KB   글 작성 요청에
im-not-ai (의존 플러그인)         설명 약 1.1k토큰이 상시, 본문은 윤문 요청에
```

**가져온 파일은 가져온 대로 둡니다.** README 스킬과 글자 수 스크립트는 다른 MIT 프로젝트의 것입니다. 어디서 가져왔고 어느 줄을 고쳤는지는 [`NOTICE.md`](./NOTICE.md)에 파일 단위로 적어 두었습니다. 윤문 규칙집은 사본을 두지 않고 im-not-ai 플러그인을 의존성으로 씁니다.
...
| 상시 컨텍스트 비용        | 이 플러그인 약 860토큰 (설명 200 + 규칙 662), im-not-ai 약 1,130토큰 |
| 외부 네트워크 호출        | 0                                                                    |
| 회귀 테스트               | 63 / 63                                                              |
...
GitHub Actions는 push와 PR마다 macOS와 Linux에서 같은 검사를 돌립니다. 매니페스트와 이슈 양식의 문법, 훅의 실행 비트, 회귀 테스트 둘, 한국어 문서 12종이 자기 훅을 통과하는지, 글자 수 스크립트의 스모크 테스트입니다. 여기에 shellcheck, `plugin.json`과 README 배지와 CHANGELOG의 버전이 같은지, `claude plugin validate`가 더해집니다.
...
- 이미 쓴 글의 윤문은 이 저장소가 직접 하지 않습니다. 의존 플러그인 im-not-ai가 합니다.
...
korean-writing/
├── .claude-plugin/
│   ├── plugin.json                   매니페스트. 이름, 버전(정본), 스킬 경로 셋, 의존성 im-not-ai
│   └── marketplace.json              카탈로그. 이 플러그인과 의존 플러그인 im-not-ai(커밋 고정) 두 항목
...
한국어 산문 품질 도구는 두 층으로 놓입니다. 규칙집을 만드는 쪽과 그 규칙을 특정 편집기에 붙이는 쪽입니다. 이 플러그인은 뒤쪽입니다. 규칙집 쪽은 im-not-ai를 의존 플러그인으로 그대로 쓰고, Claude Code에 붙이는 방식은 여기서 만들었습니다.

| 도구                                                 | 무엇인가                                                                                                            | 이 플러그인과의 관계                                                                                                        |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| [im-not-ai](https://github.com/epoko77-ai/im-not-ai) | 이미 쓴 한글에서 AI 티를 걷어내는 윤문 스킬. 진단·윤문·마무리를 나눈 다중 호출 파이프라인이고 여러 CLI를 지원합니다 | 이 플러그인의 의존성입니다. 윤문은 전부 그쪽이 맡고, 이 플러그인은 처음 쓸 때와 평소 답변, 저장할 때의 검사에 무게를 둡니다 |
...
상시로 드는 것은 이 플러그인의 스킬 설명 셋 약 200토큰과 세션당 한 번 들어가는 상시 규칙 약 660토큰, 그리고 의존 플러그인 im-not-ai의 스킬 설명 약 230토큰과 에이전트 9종 설명 약 900토큰입니다. 상시 규칙은 설치된 플러그인을 그대로 두고 상시 규칙만 껐다 켜며 `claude -p`의 입력 토큰 총량을 뺀 값으로, 끄면 12,364, 켜면 13,026이었습니다. MCP 도구 정의와 사용자 설정을 빼야 값이 고정됩니다. 규칙은 프롬프트 캐시에 들어가 턴마다 다시 내지 않습니다. 스킬 본문은 글 작성 요청이 있을 때만 로드되고 훅은 LLM을 부르지 않습니다.
...
먼저 `claude plugin list`에서 `korean-writing`과 im-not-ai의 `humanize-korean`이 `enabled`로 나오는지 봅니다. 뒤쪽이 없으면 `claude plugin install korean-writing`을 다시 실행합니다. 저장소를 `~/.claude/skills/`에 링크해 둔 채로 마켓플레이스 설치까지 했다면 둘 중 하나를 지웁니다. 스킬 목록은 세션을 새로 열어야 바뀝니다.
...
나머지는 이 저장소에서 썼습니다. `korean-writing` 스킬, 상시 규칙, 검사 훅 전체, 정답 데이터, 검증 기준이 그것입니다. 가져온 파일의 라이선스는 전부 MIT이고 원 저작권 표시는 [`NOTICE.md`](./NOTICE.md)에 모아 두었습니다. 윤문 플러그인 im-not-ai는 파일을 복사하지 않고 의존성으로 설치하므로 저작권 표시는 그 저장소에 있습니다. 이 저장소의 라이선스도 [MIT](./LICENSE)입니다.
