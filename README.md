...
- 상시 규칙의 효과 측정은 단일 턴이었습니다. 긴 대화에서 효과가 약해지는지는 재지 않았습니다.
- **서브에이전트에는 주입이 전달되지 않습니다.** 같은 방식의 SessionStart 훅에 추측할 수 없는 문자열을 넣고 서브에이전트에게 물어 두 번 확인했고, 두 번 다 못 봤다고 답했습니다. 서브에이전트가 쓰는 한국어는 지금 상시 규칙 밖에 있습니다.
...
상시로 드는 것은 스킬 설명 넷 약 290토큰과 세션당 한 번 들어가는 상시 규칙 약 660토큰입니다. 상시 규칙은 설치된 플러그인을 `KOREAN_WRITING_ALWAYS_ON_DISABLED=1`로 껐다 켜며 `claude -p`의 입력 토큰 총량을 뺀 값으로, 끄면 12,364, 켜면 13,026이었습니다. 세션 시작 컨텍스트가 1만 토큰 남짓이니 5%쯤 늘어납니다. 규칙은 프롬프트 캐시에 들어가 턴마다 다시 내지 않습니다. 스킬 본문은 글 작성 요청이 있을 때만 로드되고 훅은 LLM을 부르지 않습니다.
