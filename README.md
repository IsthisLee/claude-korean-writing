...
프롬프트로 그때그때 막으면 세션마다 다시 붙여야 하고 다 쓴 뒤에 고쳐 달라고 하면 이미 늦습니다. 다 쓴 글을 윤문해도 틀은 거의 그대로 남습니다. 블라인드 판정에서 윤문이 바꾼 글자는 0~18%였고(아래 비교의 공지는 0%) 처음부터 규칙대로 쓴 글이 사후 윤문본을 56쌍에서 49승 0패로 이겼습니다. 그래서 글이 만들어지는 시점을 셋으로 나누고 시점마다 하나씩 맡겼습니다. 설치하면 셋이 함께 켜지고 무엇을 언제 부를지 외울 일은 없습니다.
...
왼쪽은 플러그인 없이 쓴 공지를 im-not-ai 로 윤문한 결과입니다. 윤문 전과 한 글자도 다르지 않아서 굵은 글씨와 번호 목록, 본문 뒤의 분량 보고가 그대로 남았습니다. 오른쪽은 같은 요청을 `korean-writing` 스킬로 처음부터 쓴 글이고 조건을 모르는 판정자가 순서를 바꿔 물은 두 번 모두 이쪽을 골랐습니다. 한 쌍의 예이고 전체 판정은 [검증](#검증)에 있습니다. 그림은 `tools/render-before-after.py` 가 `docs/samples/before-after/` 의 실제 생성물로 그립니다.
...
6. **이미 있는 초안은 윤문합니다.** `/korean-writing:humanize` 입니다. 새 글이라면 다 쓴 뒤 윤문하는 것보다 처음부터 규칙대로 쓰는 쪽이 판정에서 이겼습니다(56쌍 49승 0패).
