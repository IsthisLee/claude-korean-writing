...
- **확인 창에 「적용하고 설명은 넉넉히」 선택지를 넣었습니다.** 규칙을 켜면 요청에 없던 곁가지 설명이 줄어듭니다(J3). 그 손실을 되돌리는 쪽을 고를 수 있게 했습니다. 고른 대로 글이 달라지는지 실측으로 확인했습니다. 디바운스 설명 문서 여덟 편에서 항목 보존이 규칙만 켰을 때 62/88 이었고 이 선택지를 고르니 72/88 로 규칙을 끈 것과 같아졌습니다(순열검정 p=0.014). 한글 중앙값도 461자에서 564자로 돌아왔습니다. 공지와 장애 보고 과제는 세 조건 모두 만점이라 차이를 보지 못했습니다. 결과가 S 와 구별되지 않으면 선택지를 도로 빼기로 미리 정해 두었고, 그 기준을 넘겨서 남깁니다
- **스킬을 적용할지 물을 때 Claude 가 제 의견과 추천을 함께 보여 줍니다.** 질문 끝에 이 글에 규칙이 맞는지 한 문장으로 적고 추천하는 쪽 선택지 끝에 「 (추천)」 을 답니다. 남이 읽을 글이면 「적용」 을, 곁가지 설명까지 길게 필요한 참고 문서면 「적용 안 함」 을 추천합니다. 글이 짧아질 수 있다는 안내 문구는 J3 에서 잰 그대로 두었습니다
...
- **쓰기 전 확인 창에서 「적용 안 함」 을 골라도 저장할 때마다 검사 훅이 문체를 지적하던 문제를 고쳤습니다.** 방금 거절한 규칙을 같은 플러그인이 계속 들이미는 꼴이었습니다. 이제 검사 훅이 세션 기록에서 가장 최근 답을 읽어 「적용 안 함」 이면 그때 손댄 `.md` 에는 알리지 않습니다. 범위는 그 파일 하나입니다. 거절과 무관한 문서는 평소대로 검사하고 다음에 「적용」 을 고르면 검사도 돌아옵니다. 걸린 항목이 있을 때만 기록을 읽으므로 평소 경로의 비용은 그대로이고, `scripts/check.sh` 로 직접 부른 검사는 기록을 주지 않아 조용해지지 않습니다
