...
emit("deny", "\n".join([
    ...
    "- 둘째 선택지 label 「적용 안 함」, description 「규칙 없이 평소대로 씁니다.」",
    "- 추천하는 쪽 label 끝에 「 (추천)」 을 붙인다. 남이 읽을 글이면 「적용」 이고 기본값이나 동작 원리 같은 "
    "곁가지 설명까지 길게 필요한 참고 문서면 「적용 안 함」 이다.",
    "- question 끝에 네 의견을 한 문장 덧붙인다. 이 글이 어떤 글이라서 그 쪽을 추천하는지만 적고 "
    "확인하지 않은 것을 단정하지 않는다.",
    "- 「적용」 이면 이 스킬을 다시 부르고 다른 답이면 스킬 없이 하던 작업을 이어 간다.",
    NO_ASK,
]))
'
exit 0
