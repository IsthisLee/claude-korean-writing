...
무엇을 하는지 다섯 줄로 줄이면 이렇습니다.

- 세션이 열릴 때 한국어 답변 규칙 아홉 항목을 넣어, 스킬이 뜨지 않는 평소 답변까지 맡습니다.
- 글을 써 달라는 요청에는 `korean-writing` 스킬이 떠서 처음부터 번역투와 AI 관용구 없이 씁니다.
- `.md` 파일을 저장하면 훅이 방금 쓴 부분을 여덟 가지 패턴으로 검사해 알립니다. 고치지는 않습니다.
- "AI 티 없애줘" 같은 요청에는 im-not-ai에서 내장한 윤문 파이프라인이 사실은 그대로 두고 문체만 고칩니다.
- 글자 수는 스크립트가 세고, README는 절 구성을 잡는 스킬이 먼저 나섭니다.
...
| 써 둔 글 다듬기 | `humanize-korean` 스킬 (im-not-ai 내장) | 다듬어 달라는 요청이 오면       | `skills/humanize-korean/SKILL.md`    |
...
| 상시 컨텍스트 비용        | 약 1,390토큰 (상시 규칙 662 + 스킬 설명 넷 430 + 에이전트 셋 297) |
| 외부 네트워크 호출        | 0                                                                 |
| 회귀 테스트               | 63 / 63                                                           |
...
- 윤문 파이프라인은 이 저장소가 만든 것이 아니라 im-not-ai의 것을 그대로 내장한 것입니다. 그 품질은 그쪽 실측에 기댑니다.
...
상시로 드는 것은 스킬 설명 넷 약 430토큰(내장한 윤문 스킬 230 포함), 에이전트 셋 약 300토큰, 세션당 한 번 들어가는 상시 규칙 약 660토큰입니다. 상시 규칙은 설치된 플러그인을 그대로 두고 상시 규칙만 껐다 켜며 `claude -p`의 입력 토큰 총량을 뺀 값으로, 끄면 12,364, 켜면 13,026이었습니다. MCP 도구 정의와 사용자 설정을 빼야 값이 고정됩니다. 규칙은 프롬프트 캐시에 들어가 턴마다 다시 내지 않습니다. 스킬 본문은 글 작성 요청이 있을 때만 로드되고 훅은 LLM을 부르지 않습니다.
