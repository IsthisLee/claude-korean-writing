...
프롬프트로 그때그때 막으려면 세션마다 다시 붙여야 합니다. 다 쓴 글을 나중에 다듬게 해도 크게 달라지지 않습니다. 실제로 다듬게 해 보니 바뀐 부분이 많아야 글의 18%였고 아래 비교의 공지는 한 글자도 바뀌지 않았습니다. 같은 요청을 처음부터 규칙대로 쓴 글과 다 쓰고 다듬은 글로 56번 견줘 보니 어느 쪽인지 모르는 AI 판정자가 처음부터 쓴 글을 49번 골랐고 다듬은 글을 고른 적은 없었습니다. 그래서 글이 만들어지는 시점을 셋으로 나누고 시점마다 하나씩 맡겼습니다. 설치하면 셋이 함께 켜지고 무엇을 언제 부를지 외울 일은 없습니다.
...
<p align="center"><img src="docs/before-after.svg" alt="같은 공지 요청을 플러그인 없이 쓰고 윤문한 글과 스킬로 처음부터 쓴 글" width="100%"></p>

왼쪽은 플러그인 없이 쓴 공지를 im-not-ai 로 윤문한 결과입니다. 윤문 전과 한 글자도 다르지 않아서 굵은 글씨와 번호 목록, 본문 뒤의 분량 보고가 그대로 남았습니다. 오른쪽은 같은 요청을 `korean-writing` 스킬로 처음부터 쓴 글입니다. 어느 글이 어느 쪽인지 알리지 않고 AI 판정자에게 두 번 물었는데(두 번째는 순서를 바꿔서) 두 번 다 오른쪽을 골랐습니다. 이 그림은 한 번의 예이고 여러 번 견준 결과는 [검증](#검증)에 있습니다. 그림은 `tools/render-before-after.py` 가 `docs/samples/before-after/` 의 실제 생성물로 그립니다.
...
4. **묻는 창에서 「적용」 을 고릅니다.** 곁가지 설명까지 길게 필요하면 「적용 안 함」 을 고릅니다.
5. **`.md` 파일로 저장하게 합니다.** 검사 훅이 걸린 자리를 줄 번호와 함께 돌려주고 Claude 가 같은 턴에 고칩니다([실험](./docs/experiments/hook-loop/)).
6. **이미 있는 초안은 `/korean-writing:humanize` 로 다듬습니다.** 새로 쓰는 글이라면 다 쓰고 다듬는 것보다 처음부터 규칙대로 쓰는 편이 낫습니다. 두 방식을 56번 견줬을 때 처음부터 쓴 글이 49번 뽑혔고 다듬은 글이 뽑힌 적은 없었습니다.
...
규칙의 핵심은 사람이 말하듯 쓰는 것이고 영어 문장을 옮긴 티가 나면 실패입니다. 시점이 중요합니다. 첫 문장부터 그렇게 쓰지, 다 써 놓고 손보지 않습니다. 다 쓴 글을 나중에 윤문하면 번호 목록이나 굵은 글씨 같은 틀이 그대로 남기 쉽습니다([같은 요청, 두 결과](#같은-요청-두-결과)).
...
| 스킬 대 im-not-ai 윤문본  | 블라인드 56쌍에서 49승 0패. 문체 따르기 절을 넣은 v2.0.0 판은 기본 네 쌍에서 3승 1패 |
