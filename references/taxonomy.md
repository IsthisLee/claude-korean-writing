...
# Korean AI-Tell Taxonomy v2.1 (AI 한글 티 분류 체계)

> **For non-Korean readers**: this is a reference catalog of "AI-tell" patterns — phrasing that reads as machine-translated or LLM-generated to a native Korean reader, even when grammatically correct. It exists because Korean has its own set of translation-ese and AI-idiom tells that don't map 1:1 onto English AI-writing tells, so a Korean-specific catalog is needed alongside general prose-quality advice. It groups ~50 sub-patterns into 10 top-level categories (A–J), each with a definition, signature example, severity tier (S1 critical / S2 strong / S3 weak), and a rewrite prescription. The body below is intentionally kept in Korean, because the patterns themselves — and the phrasing they're contrasted against — are Korean-language phenomena that lose their meaning in translation. `rules/korean-writing-quality.md` is the rule that points here; it explains, in English, what this pack does and when to use it.
>
> This file is ported from an internal harness's taxonomy of the same name. A handful of entries carry `see_scholarship: scholarship.md#...` pointers and reference `_workspace/`, `pattern-candidates.md`, `promotion-checklist.md`, and `metrics_v2.py` — those internal companion files (extended academic citations, a candidate-pattern review pipeline, and a Python metrics engine) are **not included in this port**. Treat those pointers as informational provenance only; the `_source_anchor` academic citations themselves (Korean linguistics papers, cited by author/year) are real published research and remain useful even without the companion file.
...
> **v2.1 추가 (2026-07-02):** 첫 상용 출판 실증 회차. 외부 출판사 편집부가 단행본 원고 1~5장 검토에서 "요즘 집필 제안 원고들에 위와 같은 표현이 공통적으로 나온다"며 AI 생성 의심 표현을 명시 지적(외부 인간 편집자가 실제 출판 파이프라인에서 AI-tell을 인지한 첫 외부 실증). **본진 보강 1건** — `J-3` 대시(—)·줄표 삽입구 남용 [S2→**S1 상향**, 한국어 산문에서 가장 눈에 띄는 AI-tell로 실증, 대외 원고는 0회 기본]. **본진 신규 2건** — `D-8` 승패 의인화 "X가 Y를 이긴다/이깁니다" [S2] · `D-9` 추상 구조어 남발 '축·갈래·결·레이어' [S2]. `A-17` hold는 유지(NMT 원본 회차 대기). valid as of 2026-07.
>
...
D. AI 특유의 관용구 (Signature Phrases) — D-1~D-9
...
### D-8. 승패 의인화 "X가 Y를 이긴다/이깁니다" [S2] · v2.1 신규
- 패턴: 규칙·설정·개념 사이의 우선순위를 무생물끼리의 승패 구도로 의인화. 영어 `X beats Y / X wins over Y` 감각의 직역이며, AI가 원칙을 격언처럼 보이게 하려 할 때 반복 생성.
- 예: "구체적인 것이 일반적인 것을 **이긴다**" / "프로젝트 설정이 전역 설정을 **이깁니다**" / "명시가 암묵을 **이긴다**"
- **실증 (2026-07-02)**: 외부 출판 편집자가 서로 다른 저자의 투고 원고 2건에서 같은 자리(우선순위 설명)에 같은 "이깁니다"가 등장한다고 지적하며 AI 생성 의심 표현으로 명시. 저자가 달라도 같은 위치에 같은 표현이 재현되는 것 자체가 집단 시그니처.
- 처방: 우선순위 서술로 직결한다("구체적인 규칙이 일반 규칙보다 우선한다" / "프로젝트 설정이 전역 설정에 우선 적용된다"). 한 문서 1회 이하, 대외 원고(출판·기고)는 0회 권장.

### D-9. 추상 구조어 남발 '축·갈래·결·레이어' [S2] · v2.1 신규
- 패턴: 논지의 구조를 '축이 하나 더 있다', '세 갈래', '결이 다르다', '레이어가 다르다' 같은 추상 구조 은유로 반복 서술. 낱개로는 자연스러운 한국어지만, AI 산문에서는 밀도가 비정상적으로 높아 그 자체로 인상을 남긴다.
- 예: "고려할 **축**이 하나 더 있다" / "선택지는 크게 세 **갈래**다" / "두 문제는 **결**이 다르다" / "이건 별도 **레이어**의 문제다"
- **실증 (2026-07-02)**: 외부 출판 편집자가 '축', '갈래' 류 표현을 줄표 삽입구(J-3)·"이긴다"(D-8)와 같은 회차에 AI 생성 의심 표현으로 지적.
- 처방: 구체 명사로 치환("기준·조건·경우·종류·단계") 또는 문장 재구성("고려할 기준이 하나 더 있다" / "선택지는 크게 세 가지다"). 4종 합산 한 문서 2회 이하.
...
### J-3. 대시(—)·줄표 삽입구 남용 [S1] · v2.1 상향
- 패턴: 영어 em-dash 구문 직역. 문장 중간에 줄표 쌍으로 부가 설명을 끼워 넣는 삽입구가 대표형이며, 한국어 산문에서 가장 눈에 띄는 AI-tell.
- 예: "AI는 도구 — 그 이상도 이하도 아닌 — 이다" / "어느 쪽이 좋은지 — 내 모든 일에 두루 쓸지 — 는 따져봐야 한다"
- **실증 (2026-07-02, S2→S1 상향 근거)**: 외부 출판 편집자가 상용 단행본 원고 검토에서 "요즘 집필 제안 원고들에 위와 같은 표현이 공통적으로 나온다. 잘못된 표현은 아니지만 불필요한 의심(AI 생성 의심)을 살 수 있어 가급적 쓰지 않는 편이 좋겠다"며 줄표 삽입구를 명시 지적.
- 처방: 쉼표·괄호·문장 분리로 분해. 대외 원고(출판·기고·제안)는 줄표 삽입구 전면 제거(0회)가 기본, 내부 문서도 1문서 1~2회 이하.
...
- **v2.1** (2026-07-02): **본진 신규 2건 + 본진 보강 1건** — 첫 상용 출판 실증 회차. 외부 출판사 편집부가 단행본 원고 1~5장 검토에서 AI 생성 의심 표현을 명시 지적("요즘 집필 제안 원고들에 공통적으로 나온다"):
  - **본진 보강**: `J-3` 대시(—)·줄표 삽입구 남용을 [S2→S1] 상향 + 실증 예문·처방 보강(대외 원고는 줄표 삽입구 0회 기본). 영어 em-dash 구문 직역으로, 한국어 산문에서 가장 눈에 띄는 AI-tell로 확인.
  - **본진 신규**: `D-8` 승패 의인화 "X가 Y를 이긴다/이깁니다" [S2] · `D-9` 추상 구조어 남발 '축·갈래·결·레이어' [S2]. D-8은 서로 다른 저자의 원고 2건에서 같은 자리에 같은 표현("이깁니다")이 재현된 강한 실증.
  - **A-17 hold 유지**: NMT 원본 출력 회차 미수행으로 재평가는 다음 회차로 이월.
  - **분류 체계의 새 차원**: v1.1~v1.5가 내부 재감사, v1.6~v2.0이 학술·정량 연구 기반이었다면 v2.1은 외부 인간 편집자가 실제 출판 파이프라인에서 AI-tell을 인지한 첫 실증 회차. "잘못된 표현이 아니어도 AI 생성 의심을 유발하면 회피"라는 판정 기준을 확립.
